...
def calculateDistance(point1, point2):
    x1, y1 = point1
    x2, y2 = point2
    return round(np.sqrt((x1 - x2) ** 2 + (y1 - y2) ** 2), 6)
...
def calculateCosine(point, origin):
    ...
    if y1 == y0:
        return 1.0
    return round((x1 - x0) / calculateDistance(point, origin), 6)
    ...
    for i in range(3, len(combined_points)):
        while True:
            ...
            if np.cross(old_vector, new_vector) < 0:  # neg cross = clock-wise
                stack.pop(-1)
                if DEBUG:
                    print(f"clock wise, we pop current point = {cur_point}")
            else:
...
print("final result = ", granhamScan(array))
